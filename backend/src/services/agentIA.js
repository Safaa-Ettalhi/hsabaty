...
class ServiceAgentIA {
    constructor() {
        ...
        if (this.provider === 'gemini') {
            ...
            this.anthropic = new sdk.Anthropic({
    ...
    detecterLangue(message) {
        ...
        if (englishKeywords.some((k) => lower.includes(k)))
            return 'en';
    ...
    async traiterMessage(utilisateurId, messageUtilisateur, conversationId) {
        const langue = this.detecterLangue(messageUtilisateur);
        const contexte = await this.obtenirContexteUtilisateur(utilisateurId);
        contexte.langue = langue;
        let conversation = null;
        if (conversationId && mongoose.Types.ObjectId.isValid(conversationId)) {
            conversation = await Conversation.Conversation.findOne({ _id: conversationId, utilisateurId });
        }
        ...
        try {
            ...
            if (this.provider === 'openai' && this.openai) {
                ...
                if (choix.message.function_call) {
                    action = await this.executerAction(utilisateurId, choix.message.function_call.name, JSON.parse(choix.message.function_call.arguments || '{}'));
                    if (action) {
                ...
                try {
                    const history = messagesHistorique.map(msg => ({
                        role: msg.role === 'utilisateur' ? 'user' : 'model',
            ...
            if ((isQuotaError || error?.status === 503 || msg.includes('503') || msg.includes('Service Unavailable')) && this.provider === 'gemini') {
                if (isQuotaError && process.env.OPENROUTER_API_KEY && process.env.OPENROUTER_FALLBACK === 'true') {
                    ...
                    try {
                        ...
                        if (conversationId && mongoose.Types.ObjectId.isValid(conversationId)) {
                            conversationFallback = await Conversation.Conversation.findOne({ _id: conversationId, utilisateurId });
                        }
                        const messagesHistorique = conversationFallback?.messages.slice(-10).map((msg) => ({
                            role: msg.role,
                            contenu: msg.contenu
                        })) || [];
            ...
            if (error?.status === 429 || msg.includes('429') || msg.toLowerCase().includes('quota')) {
                throw new Error("Quota de l'API IA dépassé. Vérifiez votre forfait et la facturation sur le portail du fournisseur (OpenAI, Anthropic ou Google AI Studio pour Gemini), ou réessayez plus tard.");
            }
            if (error?.status === 404 || msg.includes('404') || msg.toLowerCase().includes('not found')) {
                if (this.provider === 'gemini') {
                    throw new Error(`Modèle Gemini "${this.model}" non trouvé. Modèles valides: gemini-flash-latest, gemini-3-flash-preview, gemini-3-pro-preview, gemini-pro-latest. Vérifiez IA_MODEL dans votre .env.`);
                }
            }
            throw new Error(`Erreur de l'agent IA: ${msg}`);
        }
    }
    async genererConseils(prompt) {
    ...
    extraireActionDepuisReponse(reponseIA) {
    ...
    async executerActionsBatch(utilisateurId, actionParsee, contexte) {
    ...
    construirePromptSystem(contexte) {
        const langue = (contexte && contexte.langue) || 'fr';
...
- Associe le BON montant au BON item. Ne mélange PAS les prix.
    ...
    async obtenirContexteUtilisateur(utilisateurId) {
        ...
        const transactionsMois = await Transaction.Transaction.find({
            utilisateurId: new mongoose.Types.ObjectId(utilisateurId),
        ...
        const depensesMois = transactionsMois
            ...
            .reduce((sum, t) => sum + t.montant, 0);
        const budgetsActifs = await Budget.Budget.countDocuments({
            utilisateurId: new mongoose.Types.ObjectId(utilisateurId),
            actif: true
        });
        const objectifsActifs = await Objectif.Objectif.countDocuments({
            utilisateurId: new mongoose.Types.ObjectId(utilisateurId),
            actif: true
        });
        return {
    ...
    async executerAction(utilisateurId, nomAction, parametres) {
        switch (nomAction) {
            case 'rechercher_transactions':
                ...
                let transactionsTrouvees = await Transaction.Transaction.find(filtreRecherche)
                    .sort({ date: -1 })
                    .limit(limite)
            ...
            case 'modifier_transaction':
                ...
                const transactionAModifier = await Transaction.Transaction.findOne({
            ...
            case 'supprimer_transaction':
                ...
                const transactionASupprimer = await Transaction.Transaction.findOne({
                    _id: parametres.transactionId,
                    utilisateurId: new mongoose.Types.ObjectId(utilisateurId)
                });
                ...
                await Transaction.Transaction.findOneAndDelete({
                    _id: parametres.transactionId,
                    utilisateurId: new mongoose.Types.ObjectId(utilisateurId)
                });
                return {
            ...
            case 'supprimer_par_description': {
                const filtreSupp = { utilisateurId: new mongoose.Types.ObjectId(utilisateurId) };
                if (parametres.description) {
                ...
                const transASupprimer2 = await Transaction.Transaction.findOne(filtreSupp).sort({ date: -1 });
                if (!transASupprimer2) {
                    return {
                        type: 'erreur',
                ...
                await Transaction.Transaction.findByIdAndDelete(transASupprimer2._id);
                return {
                    type: 'transaction_supprimee',
            ...
            case 'modifier_par_description': {
                const filtreMod = { utilisateurId: new mongoose.Types.ObjectId(utilisateurId) };
                if (parametres.description) {
                ...
                const transAModifier2 = await Transaction.Transaction.findOne(filtreMod).sort({ date: -1 });
                if (!transAModifier2) {
                    return {
                        type: 'erreur',
                        message: `Aucune transaction trouvée correspondant à "${parametres.description || 'votre recherche'}".`
            ...
            case 'consulter_investissements': {
                const { Investissement: InvModel } = require('../models/Investissement');
            ...
            case 'supprimer_tous_investissements_nom_exact': {
                const { Investissement: InvSuppExact } = require('../models/Investissement');
            ...
            case 'supprimer_investissement_par_nom': {
                const { Investissement: InvModelSupp } = require('../models/Investissement');
                const uid = new mongoose.Types.ObjectId(utilisateurId);
                ...
                const supprimerTous = parametres.supprimerTous === true ||
                    parametres.tous === true ||
                    parametres.nomExact != null ||
                    nomAmbigu;
                let nomExact = parametres.nomExact != null && String(parametres.nomExact).trim()
            ...
            case 'modifier_investissement_par_nom': {
                const { Investissement: InvModelMod } = require('../models/Investissement');
                const filtreInvMod = {
                    utilisateurId: new mongoose.Types.ObjectId(utilisateurId),
                    actif: true,
            ...
            case 'creer_investissements_batch': {
                const { Investissement: InvBatch } = require('../models/Investissement');
            ...
            case 'supprimer_investissements_batch': {
                const { Investissement: InvSuppBatch } = require('../models/Investissement');
                const uid = new mongoose.Types.ObjectId(utilisateurId);
                const escape = (s) => s.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
                ...
                let noms = Array.isArray(parametres.noms)
                    ...
                    : Array.isArray(parametres.items)
                        ? parametres.items.map((x) => x?.nom).filter(Boolean)
                        : [];
                if (noms.length === 0 && parametres.nomExact != null) {
                    noms = [String(parametres.nomExact).trim()];
                }
                const nomsStr = noms.map((n) => String(n).trim()).filter(Boolean);
                const tousJetons = nomsStr.length > 0 && nomsStr.every((n) => jetonsAmbigusBatch.test(n));
                if (tousJetons) {
                    noms = ['Investissement'];
                    parametres.nomExact = 'Investissement';
                }
                let supprimerTousCorrespondants = parametres.supprimerTousCorrespondants === true || parametres.tous === true;
                ...
                const nomExactSeul = parametres.nomExact != null
                    ? String(parametres.nomExact).trim()
                ...
                for (const nom of noms) {
                    if (!nom || String(nom).trim().length < 1)
                        continue;
                    const nomStr = String(nom).trim();
                    const tousPourCeNom = typeof nom === 'object' && nom !== null && nom.supprimerTous === true;
                    const nomPourFiltre = typeof nom === 'object' && nom !== null && nom.nom != null
                        ? String(nom.nom).trim()
                        : nomStr;
                    if (tousPourCeNom || supprimerTousCorrespondants) {
                        ...
                        if (docs.length === 0)
                            erreurs.push(`Aucun exact: "${nomPourFiltre}"`);
                        continue;
                    }
                    const filtre = {
                        utilisateurId: uid,
            ...
            case 'modifier_investissements_batch': {
                const { Investissement: InvModBatch } = require('../models/Investissement');
                const items = Array.isArray(parametres.items) ? parametres.items : [];
                const modifies = [];
                const erreurs = [];
                for (const it of items) {
            ...
            case 'creer_transaction_recurrente':
                ...
                const { addDays, addMonths, addYears, addWeeks } = require('date-fns');
            ...
            case 'consulter_objectifs':
                ...
                const { ServiceCalculsFinanciers } = require('./calculsFinanciers');
                const serviceCalculs = new ServiceCalculsFinanciers();
                const objectifsAvecProgression = await Promise.all(objectifs.map(async (obj) => {
            ...
            case 'consulter_budgets':
                ...
                const moduleCalculsBudgets = require('./calculsFinanciers');
            ...
            case 'supprimer_budget_par_nom': {
                ...
                const budgetASupprimer = await Budget.Budget.findOne(filtreBudgetSupp).sort({ dateCreation: -1 });
                if (!budgetASupprimer) {
                    return {
                        type: 'erreur',
                ...
                await Budget.Budget.findByIdAndDelete(budgetASupprimer._id);
                return {
            ...
            case 'modifier_budget_par_nom': {
                ...
                const budgetAModifier = await Budget.Budget.findOne(filtreBudgetMod).sort({ dateCreation: -1 });
                if (!budgetAModifier) {
                    return {
                        type: 'erreur',
                        message: `Aucun budget trouvé correspondant à "${parametres.nom || parametres.categorie || 'votre recherche'}".`
            ...
            case 'supprimer_objectif_par_nom': {
                ...
                const objectifASupprimer = await Objectif.Objectif.findOne(filtreObjSupp).sort({ dateCreation: -1 });
                ...
                await Objectif.Objectif.findByIdAndDelete(objectifASupprimer._id);
                return {
            ...
            case 'modifier_objectif_par_nom': {
                ...
                const objectifAModifier = await Objectif.Objectif.findOne(filtreObjMod).sort({ dateCreation: -1 });
            ...
            case 'supprimer_recurrente_par_nom': {
                const { TransactionRecurrente: TRSupp } = require('../models/TransactionRecurrente');
                const filtreRecSupp = {
                    utilisateurId: new mongoose.Types.ObjectId(utilisateurId),
                    actif: true
                };
            ...
            case 'modifier_recurrente_par_nom': {
                const { TransactionRecurrente: TRMod } = require('../models/TransactionRecurrente');
                const filtreRecMod = {
                    utilisateurId: new mongoose.Types.ObjectId(utilisateurId),
                    actif: true
                };
    ...
    async sauvegarderMessage(utilisateurId, messageUtilisateur, reponseIA, action, conversationId) {
        let conversation = null;
        if (conversationId && mongoose.Types.ObjectId.isValid(conversationId)) {
            conversation = await Conversation.Conversation.findOne({ _id: conversationId, utilisateurId });
        }
    ...
    async categoriserTransaction(description) {
    ...
    detecterIntention(messageUtilisateur) {
        ...
        if (messageLower.match(/(compare|comparaison|par rapport|vs\.? | versus |mois dernier|mois précédent|mois d'avant|mois passé|l'an dernier|année dernière|même mois.*an dernier|last month|last year|year over year|yoy|évolution.*mois|différence.*mois)/i)) {
    ...
    extraireInfosTransaction(message) {
        ...
        if (messageLower.match(/(restaurant|manger|food|café|repas)/i))
            categorie = 'Alimentation';
        else if (messageLower.match(/(taxi|transport|carburant|essence|bus|métro)/i))
            categorie = 'Transport';
        else if (messageLower.match(/(loyer|électricité|eau|gaz|logement)/i))
            categorie = 'Logement';
        else if (messageLower.match(/(salaire|revenu|paie|salary|income|paycheck)/i))
            categorie = 'Salaire';
        else if (messageLower.match(/(médecin|pharmacie|santé)/i))
            categorie = 'Santé';
        else if (messageLower.match(/(école|cours|formation|éducation)/i))
            categorie = 'Éducation';
        else if (messageLower.match(/(cinéma|loisir|divertissement)/i))
            categorie = 'Divertissement';
        else if (messageLower.match(/(achat|shopping|magasin)/i))
            categorie = 'Shopping';
    ...
    extraireInfosBudget(message) {
        const messageLower = message.toLowerCase();
        const montantMatch = message.match(/(\d+(?:\s*\d+)*(?:[.,]\d+)?)\s*(?:MAD|mad|dh|dirham)/i) ||
            message.match(/(\d+(?:\s*\d+)*(?:[.,]\d+)?)/);
        const montant = montantMatch ? parseFloat(montantMatch[1].replace(/\s/g, '').replace(',', '.')) : null;
        let categorie = undefined;
        if (messageLower.match(/(alimentation|restaurant|manger|food|groceries|courses|ماكل|أكل|مطعم)/i))
            categorie = 'Alimentation';
        else if (messageLower.match(/(transport|carburant|taxi|bus|tram|نقل|طاكسي|ترانسبورت)/i))
            categorie = 'Transport';
        else if (messageLower.match(/(logement|loyer|rent|maison|appartement|كراء|السكن)/i))
            categorie = 'Logement';
        let periode = 'mensuel';
    ...
    extraireInfosObjectif(message) {
        ...
        let type = 'epargne';
        if (messageLower.match(/(urgence|fonds d'urgence|emergency)/i))
            type = 'fonds_urgence';
        else if (messageLower.match(/(remboursement|dette|debt|rembourse)/i))
            type = 'remboursement';
        else if (messageLower.match(/(voiture|car|maison|house|voyage|trip|projet|project|مشروع|سيارة|دار|سفر)/i))
            type = 'projet';
        if (!montantCible)
            return null;
        return {
    ...
    extraireInfosRecherche(message) {
        const messageLower = message.toLowerCase();
        const parametres = {};
        const montantMatch = message.match(/(\d+(?:\s*\d+)*(?:[.,]\d+)?)\s*(?:MAD|mad|dh|dirham)/i) ||
            message.match(/(\d+(?:\s*\d+)*(?:[.,]\d+)?)/);
        ...
        if (messageLower.match(/(alimentation|restaurant|manger|food)/i))
            parametres.categorie = 'Alimentation';
        else if (messageLower.match(/(transport|carburant|taxi)/i))
            parametres.categorie = 'Transport';
        else if (messageLower.match(/(logement|loyer|électricité)/i))
            parametres.categorie = 'Logement';
        else if (messageLower.match(/(santé|médecin|pharmacie)/i))
            parametres.categorie = 'Santé';
        if (messageLower.match(/(revenus|revenu|salaire)/i))
            parametres.type = 'revenu';
        else if (messageLower.match(/(dépenses|dépense)/i))
            parametres.type = 'depense';
        const descMatch = message.match(/(?:pour|au|à|chez)\s+([^0-9]+?)(?:\s+(?:hier|aujourd'hui|ce mois|ce mois-ci))?/i);
        if (descMatch)
            parametres.description = descMatch[1].trim();
        // Extraire la période
        let dateDebut;
        let dateFin;
        const maintenant = new Date();
        ...
        if (dateDebut)
            parametres.dateDebut = dateDebut.toISOString().split('T')[0];
        if (dateFin)
            parametres.dateFin = dateFin.toISOString().split('T')[0];
    ...
    extraireInfosTransactionRecurrente(message) {
        const messageLower = message.toLowerCase();
        const montantMatch = message.match(/(\d+(?:\s*\d+)*(?:[.,]\d+)?)\s*(?:MAD|mad|dh|dirham)/i) ||
            message.match(/(\d+(?:\s*\d+)*(?:[.,]\d+)?)/);
        const montant = montantMatch ? parseFloat(montantMatch[1].replace(/\s/g, '').replace(',', '.')) : null;
        ...
        if (messageLower.match(/(hebdomadaire|chaque semaine|toutes les semaines|semaine)/i))
            frequence = 'hebdomadaire';
        else if (messageLower.match(/(trimestriel|trimestre|tous les 3 mois)/i))
            frequence = 'trimestriel';
        else if (messageLower.match(/(annuel|année|chaque année|tous les ans)/i))
            frequence = 'annuel';
        let categorie = 'Autres';
        if (messageLower.match(/(netflix|spotify|disney|abonnement|streaming)/i))
            categorie = 'Divertissement';
        else if (messageLower.match(/(loyer|logement|rent)/i))
            categorie = 'Logement';
        else if (messageLower.match(/(salaire|revenu|paie|salary|income)/i))
            categorie = 'Salaire';
        else if (messageLower.match(/(électricité|eau|gaz|internet|téléphone)/i))
            categorie = 'Logement';
        else if (messageLower.match(/(gym|salle de sport|fitness)/i))
            categorie = 'Santé';
        if (/(الكراء|لكراء|الايجار|ايجار|الاجار|الاجر|كراء|إيجار)/i.test(message)) {
            categorie = 'Logement';
        }
        const descMatch = message.match(/(netflix|spotify|disney|loyer|salaire|électricité|eau|gaz|internet|téléphone|gym|salle de sport|rent|الكراء|كراء|إيجار)/i) ||
    ...
    async executerActionDetectee(utilisateurId, intention) {
        ...
        try {
            ...
            if (intention.type === 'analyser_habitudes') {
                ...
                if (repartition.length === 0) {
                    const debut3Mois = date_fns.subMonths(maintenant, 3);
                    repartitionFinale = await serviceCalculs.obtenirRepartitionDepenses(utilisateurId, debut3Mois, finMois);
    ...
    genererReponseDepuisAction(action, contexte) {
        if (!action)
        ...
        const ok = (titre, lignes) => `## ${titre}\n\n` + lignes.map((l) => `- ${l}`).join('\n');
        switch (action.type) {
            case 'transaction_ajoutee': {
                const trans = action.details;
                return t(ok('Transaction enregistrée', [
            ...
            case 'transaction_modifiee':
                return t(ok('Transaction modifiée', [action.message || 'Mise à jour effectuée.']), ok('Transaction updated', [action.message || 'Update completed.']), `## تم التعديل\n\n${action.message || ''}`);
            case 'transaction_supprimee':
                return t(ok('Transaction supprimée', [action.message || 'Suppression effectuée.']), ok('Transaction deleted', [action.message || 'Deletion completed.']), `## تم الحذف\n\n${action.message || ''}`);
            case 'transactions_trouvees':
                if (action.nombre === 0) {
                    return t('> Aucune transaction ne correspond à vos critères.\n\nEssayez d’élargir la recherche (dates, catégorie).', '> No transaction matched your criteria.', '> لا توجد نتائج.');
                }
                const rows = action.transactions
                    ...
                    .join('\n');
                return t(`## Résultats de recherche\n\n**${action.nombre}** transaction(s) trouvée(s).\n\n| Description | Montant | Type | Catégorie | Date |\n|-------------|--------:|:-----|:----------|:-----|\n${rows}`, `## Search results\n\n**${action.nombre}** transaction(s) found.\n\n| Description | Amount | Type | Category | Date |\n|-------------|--------:|:-----|:---------|------|\n${rows}`, `## نتائج البحث\n\n**${action.nombre}** عملية.\n\n${rows}`);
            case 'budget_cree': {
                const budget = action.details;
                return t(ok('Budget créé', [
                    `**Nom** : ${budget.nom}`,
                    `**Montant** : ${budget.montant} MAD`,
                    `**Période** : ${budget.periode}`,
                ]), ok('Budget created', [`**Name** : ${budget.nom}`, `**Amount** : ${budget.montant}`, `**Period** : ${budget.periode}`]), `## ميزانية جديدة\n\n- ${budget.nom} — ${budget.montant} / ${budget.periode}`);
            }
            case 'budgets_trouves':
                if (action.nombre === 0) {
                    return t('> Vous n’avez pas encore de budget.\n\nCréez-en un pour suivre vos dépenses par catégorie.', '> You have no budgets yet.', '> لا توجد ميزانيات.');
                }
                const budgetRows = action.budgets
                    .slice(0, 15)
                    .map((b) => `| ${b.nom} | ${b.montant} | ${b.categorie || '—'} | ${b.periode || '—'} | ${b.actif ? 'Oui' : 'Non'} |`)
                    .join('\n');
                return t(`## Vos budgets (${action.nombre})\n\n| Nom | Montant | Catégorie | Période | Actif |\n|-----|--------:|:----------|:--------|:-----:|\n${budgetRows}`, `## Your budgets (${action.nombre})\n\n| Name | Amount | Category | Period | Active |\n|------|--------:|:---------|:-------|:------:|\n${budgetRows}`, `## الميزانيات (${action.nombre})\n\n${budgetRows}`);
            case 'objectif_cree': {
                const obj = action.details;
                const dateStr = new Date(obj.dateLimite).toLocaleDateString('fr-FR');
                return t(ok('Objectif créé', [
                    `**Nom** : ${obj.nom}`,
                    `**Cible** : ${obj.montantCible} MAD`,
                    `**Échéance** : ${dateStr}`,
                ]), ok('Goal created', [`**Name** : ${obj.nom}`, `**Target** : ${obj.montantCible}`, `**Deadline** : ${dateStr}`]), `## هدف جديد\n\n- ${obj.nom} — ${obj.montantCible} — ${dateStr}`);
            }
            case 'investissement_cree': {
                const inv = action.details;
                return t(ok('Investissement enregistré', [`**${inv.nom}**`, `**Montant** : ${inv.montantInvesti} MAD`, `**Type** : ${inv.type}`]), ok('Investment recorded', [`**${inv.nom}**`, `**Amount** : ${inv.montantInvesti}`, `**Type** : ${inv.type}`]), `## استثمار\n\n- ${inv.nom} — ${inv.montantInvesti}`);
            }
            case 'investissements_liste': {
                const invs = action.investissements || [];
                const resume = action.resume;
                if (invs.length === 0) {
                    return t('Aucun investissement enregistré pour le moment.', 'No investments recorded yet.', 'لا توجد استثمارات مسجلة.');
                }
                const lignes = invs.slice(0, 15).map((i) => {
                    // Plusieurs entrées avec le même nom générique : distinguer par suffixe id
                    const nomAffiche = /^investissement$/i.test(String(i.nom || '').trim()) && i.id
                ...
                const resumeStr = resume
                    ...
                    : '';
                return t(ok('Vos investissements', lignes) + resumeStr, ok('Your investments', lignes) + resumeStr, `## محفظتك\n\n` + invs.slice(0, 10).map((i) => `- ${i.nom}: ${i.valeurActuelle}`).join('\n'));
            }
            case 'investissement_supprime':
                return t(ok('Investissement supprimé', [action.message || action.details?.nom || 'Suppression confirmée.']), ok('Investment deleted', [action.message || action.details?.nom || 'Confirmed.']), `## تم الحذف\n\n${action.message || ''}`);
            case 'investissement_modifie': {
                ...
                return t(ok('Investissement modifié', lignes), ok('Investment updated', lignes), `## تم التعديل\n\n${action.message || ''}`);
            }
            case 'investissements_batch_crees': {
                const n = action.nombre || 0;
                const details = action.details || [];
                const lignes = details.slice(0, 20).map((inv) => `**${inv.nom}** — ${inv.montantInvesti} MAD (${inv.type})`);
                const err = action.erreurs?.length ? `\n\n⚠ ${action.erreurs.join('; ')}` : '';
                return t(ok(`${n} investissement(s) enregistré(s)`, lignes) + err, ok(`${n} investment(s) recorded`, lignes) + err, `## تم التسجيل — ${n}\n\n` + lignes.join('\n') + err);
            }
            case 'investissements_batch_supprimes': {
                const n = action.nombre || 0;
                const details = action.details || [];
                const lignes = details.map((d) => `**${d.nom}** supprimé`);
                const err = action.erreurs?.length ? `\n\n⚠ ${action.erreurs.join('; ')}` : '';
                return t(ok(`${n} investissement(s) supprimé(s)`, lignes) + err, ok(`${n} investment(s) deleted`, lignes) + err, `## تم الحذف — ${n}\n\n` + lignes.join('\n') + err);
            }
            case 'investissements_batch_modifies': {
                const n = action.nombre || 0;
                const details = action.details || [];
                const lignes = details.map((d) => `**${d.nom}** : ${(d.modifications || []).join(', ')}`);
                const err = action.erreurs?.length ? `\n\n⚠ ${action.erreurs.join('; ')}` : '';
                return t(ok(`${n} investissement(s) modifié(s)`, lignes) + err, ok(`${n} investment(s) updated`, lignes) + err, `## تم التعديل — ${n}\n\n` + lignes.join('\n') + err);
            }
            case 'erreur':
            ...
            case 'transaction_recurrente_creee': {
                ...
                return t(ok('Récurrente créée', [`**${tr.description}**`, `**Montant** : ${tr.montant} MAD`, `**Fréquence** : ${tr.frequence}`]), ok('Recurring created', [`**${tr.description}**`, `**Amount** : ${tr.montant}`, `**Frequency** : ${tr.frequence}`]), `## متكررة\n\n- ${tr.description} — ${tr.montant} / ${tr.frequence}`);
            }
            case 'budget_supprime':
                return t(ok('Budget supprimé', [action.message || 'Suppression confirmée.']), ok('Budget deleted', [action.message || 'Confirmed.']), `## تم الحذف\n\n${action.message || ''}`);
            case 'budget_modifie':
                return t(ok('Budget modifié', [action.message || 'Mise à jour confirmée.']), ok('Budget updated', [action.message || 'Confirmed.']), `## تم التعديل\n\n${action.message || ''}`);
            case 'objectif_supprime':
                return t(ok('Objectif supprimé', [action.message || 'Suppression confirmée.']), ok('Goal deleted', [action.message || 'Confirmed.']), `## تم الحذف\n\n${action.message || ''}`);
            case 'objectif_modifie':
                return t(ok('Objectif modifié', [action.message || 'Mise à jour confirmée.']), ok('Goal updated', [action.message || 'Confirmed.']), `## تم التعديل\n\n${action.message || ''}`);
            case 'recurrente_supprimee':
                return t(ok('Récurrente supprimée', [action.message || 'Suppression confirmée.']), ok('Recurring deleted', [action.message || 'Confirmed.']), `## تم الحذف\n\n${action.message || ''}`);
            case 'recurrente_modifiee':
                return t(ok('Récurrente modifiée', [action.message || 'Mise à jour confirmée.']), ok('Recurring updated', [action.message || 'Confirmed.']), `## تم التعديل\n\n${action.message || ''}`);
            case 'comparaison_temporelle': {
                const d = action.donnees;
                if (!d || !d.courant) {
                    return t('Données de comparaison indisponibles pour le moment.', 'Comparison data is temporarily unavailable.', 'بيانات المقارنة غير متوفرة حالياً.');
                }
                const fmt = (n) => new Intl.NumberFormat('fr-MA', { maximumFractionDigits: 0 }).format(Math.round(n)) + ' MAD';
                const ligneComparaison = (prev, curr, pct, labelPrev) => {
                    if (prev === 0 && curr === 0)
                        return `- **${labelPrev}** : 0 MAD — aucune activité sur les deux périodes`;
                    if (prev === 0 && curr > 0)
                ...
                const epargneDiffStr = (diff) => (diff >= 0 ? '+' : '') + fmt(diff);
                return t(`## Comparaison temporelle\n\n` +
            ...
            case 'statistiques': {
                ...
                const tx = (stats.tauxEpargne != null && typeof stats.tauxEpargne === 'number')
                    ...
                    : '—';
                return t(`## Synthèse du mois\n\n| Indicateur | Valeur |\n|------------|--------:|\n| **Solde actuel** | ${stats.solde} |\n| **Revenus** | ${stats.revenus} |\n| **Dépenses** | ${stats.depenses} |\n| **Épargne (net)** | ${stats.epargne != null ? stats.epargne : (Number(stats.revenus) - Number(stats.depenses))} |\n| **Taux d'épargne** | ${tx}% |`, `## Monthly summary\n\n| | |\n|--|--:|\n| **Balance** | ${stats.solde} |\n| **Income** | ${stats.revenus} |\n| **Expenses** | ${stats.depenses} |\n| **Savings rate** | ${tx}% |`, `## ملخص\n\n- الرصيد: ${stats.solde}\n- الدخل: ${stats.revenus}\n- المصاريف: ${stats.depenses}\n- نسبة الادخار: ${tx}%`);
            }
            case 'objectifs_trouves':
                if (action.nombre === 0) {
                    return t('> Aucun objectif pour l’instant.\n\nCréez un objectif depuis la page **Objectifs** ou dites-moi par exemple : *« objectif vacances 10 000 d’ici décembre »*.', '> No goals yet.\n\nCreate one from the Goals page or say e.g. *"vacation goal 10000 by December"*.', '> لا أهداف بعد.');
                }
                const fmtMad = (n) => new Intl.NumberFormat('fr-MA', { maximumFractionDigits: 0 }).format(Math.round(n));
                const objectifsDetails = action.objectifs.map((o) => {
                ...
                const objectifsTexte = objectifsDetails
                    .map((o) => (langue === 'en' ? o.en : langue === 'ar' ? o.ar : o.fr))
                    .join('');
                return t(`## Vos objectifs (${action.nombre})\n\n${objectifsTexte}`, `## Your goals (${action.nombre})\n\n${objectifsTexte}`, `## أهدافك (${action.nombre})\n\n${objectifsTexte}`);
            case 'habitudes_analysees':
                if (!action.categories || action.categories.length === 0) {
                    return t('> Pas assez de dépenses enregistrées pour une analyse.\n\nAjoutez quelques transactions **dépense** puis redemandez une analyse des habitudes.', '> Not enough expenses to analyse yet.', '> بيانات غير كافية.');
                }
                ...
                const catRows = action.categories
                    .map((cat, index) => `| ${index + 1} | ${cat.categorie} | ${cat.montant.toFixed(0)} | ${cat.pourcentage.toFixed(1)}% |`)
                    .join('\n');
                return t(`## Où vous dépensez le plus (${periodeText})\n\n| # | Catégorie | Montant | Part |\n|--:|:----------|--------:|:-----|\n${catRows}\n\n*${action.categories.length} catégorie(s) prises en compte.*`, `## Where you spend most (${periodeText})\n\n| # | Category | Amount | Share |\n|--:|:---------|-------:|:------|\n${catRows}`, `## أين تنفق أكثر\n\n${catRows}`);
            default:
                return t('> Action enregistrée.\n\nSi quelque chose ne s’affiche pas comme prévu, précisez votre demande (ex. *mes budgets*, *solde du mois*).', '> Action completed.\n\nIf something looks off, try rephrasing (e.g. *my budgets*, *this month balance*).', '> تم التنفيذ.');
        }
    }
    async appelerNemotronViaOpenRouter(messagesHistorique, messageUtilisateur, promptSystem) {
...
exports.ServiceAgentIA = ServiceAgentIA;
