...
export class VectorService {
  ...
  static async genererEmbedding(texte: string): Promise<number[]> {
    if (!this.gemini) {
  ...
  static async upsertTransaction(t: ITransaction) {
    await this.init();
    if (!this.pc) return;
    ...
    console.log(`[VectorService] ✅ Transaction ${(t as any)._id} synchronisée (Pinecone)`);
  }

  static async supprimerTransaction(transactionId: string) {
    await this.init();
    if (!this.pc) return;

    try {
      const index = this.pc.index(this.indexName);
      await (index as any).deleteOne(transactionId);
  ...
  static async rechercherSimilaires(utilisateurId: string, requete: string, limite = 5) {
    await this.init();
    if (!this.pc) return [];
